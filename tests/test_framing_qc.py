...
Two tiers, clearly separated:
  1. OFFLINE (no API key, no network, runs in ~1s): synthetic good/bad
     ...
     resolution) for real, and the two vision-dependent layers -- the
     subject-validity check (not_electrode, which now runs first on every
     call, see framing_qc's module docstring for why) and the bundled
     framing check (angled, overlapping, multiple_electrodes, flipped,
     tampered, mixed_types) -- are exercised through monkeypatched
     functions. Validates the aggregation/gating logic (issue codes,
     message specificity, the early-return guard) without spending a
     Gemini call. This tier is what you can always run, key or no key,
     right before walking into a demo.
  2. LIVE (needs GEMINI_API_KEY, several real vision calls): confirms the
     actual vision integration works end to end against a real good photo,
     a real non-electrode photo, and every photo in bad_examples/. Skipped
     automatically if no key is configured, with a clear note rather than
     a silent gap.
...
def clean_not_electrode_response():
    return {"checked": True, "not_electrode": False, "not_electrode_reason": None}
...
# Subject-validity now runs first on every single call (see framing_qc's
# module docstring), so it's mocked once here for the whole offline tier --
# every case below inherits "yes, this is an electrode" unless it overrides
# it locally (only the not-electrode case itself does).
with patch.object(framing_qc, "_vision_not_electrode_check", return_value=clean_not_electrode_response()):
    ...
    check("bundled vision check was never called (deterministic check already failed)", r["measurements"]["vision"] is None)
...
# Not-electrode cases override the outer "yes, it's an electrode" patch locally --
# these deliberately run OUTSIDE the shared `with` block above.

print("\n-- not an electrode (mocked subject-validity check: not_electrode=True) --")
with patch.object(framing_qc, "_vision_not_electrode_check", return_value={"checked": True, "not_electrode": True, "not_electrode_reason": "shows a hand, not a strip"}):
    r = framing_qc.check_photo_framing(image_bytes=to_bytes(base), mime_type="image/jpeg")
check("'not_electrode' in issues", "not_electrode" in r["issues"], str(r["issues"]))
check("proceed is False", r["proceed"] is False)
check("bundled vision check never runs once subject is already rejected", r["measurements"]["vision"] is None if "vision" in r["measurements"] else True)

print("\n-- GUARD: predict_electrode_performance also gates on subject validity --")
with patch.object(framing_qc, "_vision_not_electrode_check", return_value={"checked": True, "not_electrode": True, "not_electrode_reason": "not an electrode"}):
    result = agent.AVAILABLE_FUNCTIONS["predict_electrode_performance"](image_path=str(REAL_PHOTO))
check("status is 'framing_rejected'", result.get("status") == "framing_rejected", result.get("status"))

print("\n-- subject-validity check itself failing (network/API error) doesn't wave the photo through --")
with patch.object(framing_qc, "_vision_not_electrode_check", return_value={"checked": False, "error": "simulated timeout"}):
    r = framing_qc.check_photo_framing(image_bytes=to_bytes(base), mime_type="image/jpeg")
check("proceed is False, not defaulted to True", r["proceed"] is False)
check("'framing_check_incomplete' in issues", "framing_check_incomplete" in r["issues"], str(r["issues"]))
...
if not agent.api_key_configured():
    ...
    print("\n-- real non-electrode photo (skimage sample) is correctly identified as such --")
    print("   (now guaranteed to be checked, since subject-validity runs before anything else --")
    print("   previously this could get caught by an unrelated cheap check instead)")
    skimage_astronaut = Path(sys.prefix) / "lib" / "python3.13" / "site-packages" / "skimage" / "data" / "astronaut.png"
    if skimage_astronaut.is_file():
        r = agent.AVAILABLE_FUNCTIONS["check_photo_framing"](image_path=str(skimage_astronaut))
        check("framing_ok is False", r["framing_ok"] is False, str(r["issues"]))
        check("'not_electrode' in issues", "not_electrode" in r["issues"], str(r["issues"]))
    else:
        print(f"\n(skipping non-electrode live case -- {skimage_astronaut} not found in this environment)")
